...
def test_callback_error_async(lroot, rroot):
    """Call an exception-raising local method remotely, without remote post-processing."""
    try:
...
def test_callback_error_sync(lroot, rroot):
    """Call an exception-raising local method remotely, with remote post-processing."""
...
def test_marshal_outofband(rroot_msgpack):
    ...
    with rroot_msgpack.new_Value(6) as six, rroot_msgpack.new_Value(11) as eleven:
        eleven.add(six)
        assert 17 == eleven.val()

def test_pass_oid(rroot_msgpack):
    """Pass dict with special "__*__" key (only works with out-of-band codecs)."""
